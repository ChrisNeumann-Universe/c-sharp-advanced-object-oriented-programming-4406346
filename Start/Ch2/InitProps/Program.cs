...
// emp1.ID = 5; // ID is set to init, so only first time use and/or constructor can change the ID 
Console.WriteLine(emp1);

Console.WriteLine(emp2);
emp2.LastName = "Dough";
// emp2.ID = 3; // ID is set to init, so only first time use and/or constructor can change the ID
Console.WriteLine(emp2);
